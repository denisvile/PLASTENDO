...
# Flow cytometry data ----
# €€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€
d <- read.flowSet(path=paste("../data/FCSexports/rosette_Leaf/")) 
d <- read.flowSet(path=paste("/Users/denis/Documents/Encadrements/Stages/2021\ -\ M2\ -\ Benoit\ Berthet\ -\ Endopolyploidy/Experiment/endopolyploidy/data.Endopolyploidy/Ag-0/F6/"))
                             #Essai\ Arabidopsis\ congele\ -n\ 74\ Ag0\ f6\ -\ DAPI\ 5001_006.FCS 
...
names(ddf) <- "DAPI"
log10(ddf$DAPI)
ggplot(ddf, aes(x=DAPI)) + geom_histogram(bins = 100) + 
...
# €€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€
# Global ----
res <- filter(dat, curv1Filter("lgDAPI", bwFac=0.48))
resSum <- summary(res)

dfRes <- toTable(resSum)
names(dfRes); head(dfRes); str(dfRes)
dfRes$population <- factor(dfRes$population, labels=c("peak1", "peak2", "peak3", "peak4", "peak5","peak6", "rest"))
...
split.res <- split(dat, res, population=list(keep=c("peak 1")))
...
split.res <- split(dat, res, population=list(keep=c("peak 2")))
for(i in 1:Lth){
  df.res[i, c("min2", "max2")] <- 10^(range(exprs(split.res$keep[[i]]$"lgDAPI")))
}
split.res <- split(dat, res, population=list(keep=c("peak 3")))
for(i in 1:Lth){
  df.res[i, c("min3", "max3")] <- 10^(range(exprs(split.res$keep[[i]]$"lgDAPI")))
}
split.res <- split(dat, res, population=list(keep=c("peak 4")))
for(i in 1:Lth){
  df.res[i, c("min4", "max4")] <- 10^(range(exprs(split.res$keep[[i]]$"lgDAPI")))
}
split.res <- split(dat, res, population=list(keep=c("peak 5")))
for(i in 1:Lth){
  df.res[i, c("min5", "max5")] <- 10^(range(exprs(split.res$keep[[i]]$"lgDAPI")))
}
split.res <- split(dat, res, population=list(keep=c("peak 6")))
for(i in 1:Lth){
  df.res[i, c("min6", "max6")] <- 10^(range(exprs(split.res$keep[[i]]$"lgDAPI")))
}
split.res <- split(dat, res, population=list(keep=c("peak 7")))
for(i in 1:Lth){
...
# €€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€€
pdf("Fluorescence_peaks.pdf", 7, 7)
...
dev.off()
system(paste("open", "Fluorescence_peaks.pdf"))
